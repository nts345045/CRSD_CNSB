...
T24_NT = os.path.join(DDIR,'5_split_data','EX_T24-Pressure.csv')
T24_LV = os.path.join(DDIR,'6_lvdt_melt_corrected','EX_T24-LVDT-reduced.csv')
T24_CM = os.path.join(DDIR,'cavities','experiment_T24_cavity_metrics.csv')
T24_CP = os.path.join(DDIR,'cavities','Postprocessed_Cavity_Geometries.csv')
...
df_CM24.index += pd.Timedelta(5,unit='hour')
df_CP24.index += pd.Timedelta(5,unit='hour')
...
# Plot modeled values
axs[0].plot(dtzindex,df_CM24['hat T kPa'].values[:-1],'r--',zorder=5)
...
# plot modeled values
axs[1].plot(dtzindex,mu_calc[:-1],'r--',zorder=5,label='Mod.')
...
axs[2].plot(dtzindex,df_CM24['S tot'].values[:-1],'b-',zorder=10)
# Plot modeled values
axs[2].plot(dtzindex,df_CM24['hat S tot'].values[:-1],'r--',zorder=5)
