...
:license: CC-BY-4.0
:purpose: Plot the phase spaces for N-\\mu-S 
...
def main(args):
	...
	### SUBPLOT (C) T24 N x \mu
	# Set data and model vectors
	XI = df_T24['N kPa'].values
	XM = df_MOD.index.values*1e-3
	YI = df_mu_T24.values
	YM = df_mu_calc.values
	II = df_T24.index
	...
	axs[3].text(330,-0.05+0.23,'- Rising $N$ ->', rotation=-8)
	axs[3].text(310,0.03 + 0.23, '<- Falling $N$ -', rotation=-10)

	### SUBPLOT (E) T24 S x \mu
	# Get data and modeled value vectors
	...
	axs[0].set_ylabel('Contact Fraction [$S^{LVDT}$]( - )')
	# Add drag labels
	for _i in [2,4]:
		axs[_i].set_ylabel('Drag [$\mu^{obs}$] ( - )')
