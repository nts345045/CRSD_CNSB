"""
:module: S2_smooth_despike_and_segment_data.py
:purpose: This script regularly samples data time-series at near-acquisition-equivalent
		  sampling rates, regularizing sampling times across the experiment and filling 
		  minor data gaps that arise in the course of processing. The script then segments
		  the data into defined experimental windows for experiment T24, T6, and T96. 
		  Segmented results are saved to CSV.
...
FDIR = os.path.join(ROOT,'results','figures','supplemental')
# Data files
FNTP = os.path.join(DDIR,'S2_NTP_Local_PD_DateTime.csv')
FLVDT = os.path.join(DDIR,'S2_LVDT_Local_PD_DateTime_FULL.csv')
# Load Data
df_NTP = pd.read_csv(FNTP,parse_dates=True,index_col=[0])
df_LVDT = pd.read_csv(FLVDT,parse_dates=True,index_col=[0])
...
# Define Smoothing Operator Parameters
siter = 3
smoothwin_min =10
...
axs[2].legend()
axs[3].plot(df_LVDTr['LVDT_mm_stitched'],label='shift corrected')
axs[3].plot(df_LVDTrd['LVDT_mm_stitched'],label='despiked')
...
plt.show()
if issave:
	OFILE = os.path.join(FDIR,'CNSB_FigS1_Raw_Timeseries_%ddpi.%s'%(DPI,FMT.lower()))
...
# Conduct multiple smoothing operations, if programmed.
if siter > 1:
...
df_NTrds.to_csv(os.path.join(DDIR,'S3_FULL_NT_%smin_x%s_smoothed.csv')%(smoothwin_min,siter))
df_LVDTrds.to_csv(os.path.join(DDIR,'S3_FULL_LVDT_%smin_x%s_smoothed.csv')%(smoothwin_min,siter))
...
df_NTS1.to_csv(os.path.join(DDIR,'S3_SS1_NT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_NT24 = df_NTrds[(df_NTrds.index >= TSV[1] - DT_PAD) & (df_NTrds.index <= TEV[1] + DT_PAD)]
df_NT24.to_csv(os.path.join(DDIR,'S3_T24_NT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_NTS2 = df_NTrds[(df_NTrds.index >= TSV[2] - DT_PAD) & (df_NTrds.index <= TEV[2] + DT_PAD)]
df_NTS2.to_csv(os.path.join(DDIR,'S3_SS2_NT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_NT06 = df_NTrds[(df_NTrds.index >= TSV[3] - DT_PAD) & (df_NTrds.index <= TEV[3] + DT_PAD)]
df_NT06.to_csv(os.path.join(DDIR,'S3_T06_NT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_NT96 = df_NTrds[(df_NTrds.index >= TSV[4] - DT_PAD) & (df_NTrds.index <= TEV[4] + DT_PAD)]
df_NT96.to_csv(os.path.join(DDIR,'S3_T96_NT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))


df_LVDTS1 = df_LVDTrds[(df_LVDTrds.index >= TSV[0] - DT_PAD) & (df_LVDTrds.index <= TEV[0] + DT_PAD)]
df_LVDTS1.to_csv(os.path.join(DDIR,'S3_SS1_LVDT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_LVDT24 = df_LVDTrds[(df_LVDTrds.index >= TSV[1] - DT_PAD) & (df_LVDTrds.index <= TEV[1] + DT_PAD)]
df_LVDT24.to_csv(os.path.join(DDIR,'S3_T24_LVDT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_LVDTS2 = df_LVDTrds[(df_LVDTrds.index >= TSV[2] - DT_PAD) & (df_LVDTrds.index <= TEV[2] + DT_PAD)]
df_LVDTS2.to_csv(os.path.join(DDIR,'S3_SS2_LVDT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_LVDT06 = df_LVDTrds[(df_LVDTrds.index >= TSV[3] - DT_PAD) & (df_LVDTrds.index <= TEV[3] + DT_PAD)]
df_LVDT06.to_csv(os.path.join(DDIR,'S3_T06_LVDT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))

df_LVDT96 = df_LVDTrds[(df_LVDTrds.index >= TSV[4] - DT_PAD) & (df_LVDTrds.index <= TEV[4] + DT_PAD)]
df_LVDT96.to_csv(os.path.join(DDIR,'S3_T96_LVDT_%smin_x%s_smoothed.csv'%(smoothwin_min,siter)))
