"""
:module: JGLAC_Fig08_Exp_T06_Crossplots.py
...
:Figure #: 8
:Figure Caption: Parameter cross-plots for experiment T6
					(a–b) \\tau(N(t))
					(c–d) \\Delta \\mu(N(t))
					(e–f) \\tau(S(t))
					(g-h) \\Delta \\mu(S(t)) 
...
# Load Data/Model Values
df_T06 = pd.read_csv(T6_CM,parse_dates=True,index_col=[0])
# df_T06 = df_T06.resample(pd.Timedelta(1,unit='sec')).interpolate(method='linear')
df_MOD = pd.read_csv(MOD_CM,parse_dates=True,index_col=[0])

# Hard set starttime for Exp. T06
...
axs[0].text(265,81,'$-\\partial_t N$',rotation=45,ha='center',va='center')
axs[0].arrow(311.8,83.47,252.1-311.8,72.44-83.47,head_width=2,width=0.1,head_length=10,fc='k')
axs[0].text(317.6,60.56,'$+\\partial_t N$',rotation=55,ha='center',va='center')
axs[0].arrow(259.1,53.7,346.2 - 259.1,71.87 - 53.7,head_width=2,width=0.1,head_length=10,fc='k',zorder=100)
...
axs[1].set_xlabel('Scaled Contact Length [$S$] ( - )')
...
axs[1].text(.1995,82.43,'$+\\partial_t N$',rotation=45,ha='center',va='center')
axs[1].text(.21115,66.88,'$-\\partial_t N$',rotation=50,ha='center',va='center')

### (c) Plot T6 \Delta \mu(N(t)) ###
XI = df_T06['N kPa'].values
mu_obs = (df_T06['T kPa'].values - D_tau)/XI
...
### (d) Plot T6 \mu(S(t)) ###
XI = df_T06['S tot'].values
mu_obs = (df_T06['T kPa'].values - D_tau)/df_T06['N kPa'].values
YI = mu_obs
II = df_T06.index
xlims = (np.nanmin(XI) - PADXY*(np.nanmax(XI) - np.nanmin(XI)),\
		 np.nanmax(XI) + PADXY*(np.nanmax(XI) - np.nanmin(XI)))
...
if issave:
	SAVE_FILE = 'JGLAC_Fig08_v1.6_T06_Crossplots_%ddpi.%s'%(DPI,FMT.lower())
