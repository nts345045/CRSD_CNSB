...
if [ ! -d $DDIR ]; then
    echo "data directory $DDIR does not exist - exiting"
    break
fi

if [ ! -d $PDDIR ]; then
    echo "processed_data directory $PDDIR does not exist, making directory"
    mkdir -p $PDDIR
fi


if [ ! -d $RDIR]; then
    echo "results directory $RDIR does not exist, making directory"
    mkdir -p $RDIR
fi

if [ ! -d $FIGDIR]; then
    echo "figure saving directory $FIGDIR does not exist, making directory"
    mkdir -p $FIGDIR
fi
...
STRESS_RAW_FILE="$DDIR/RS_RAWdata_OSC_N.mat"
STRESS_PUTC_FILE="$PDDIR/0_ingestion/PhysicalUnitsData__UTC_Timing.csv"
...
LVDT_RAW_FILES="$DDIR/LVDT/*.txt"
LVDT_UTC_FILE="$PDDIR/0_ingestion/Stitched_LVDT_Data__UTC_Timing.csv"
...
STRESS_RESAMPLED="$PDDIR/1_cleanup/Resampled_Pressure_Data.csv"
LVDT_RESAMPLED="$PDDIR/1_cleanup/Resampled_LVDT_Data.csv"
...
STRESS_DESPIKED="$PDDIR/1_cleanup/Despiked_Pressure_Data.csv"
LVDT_DESPIKED="$PDDIR/1_cleanup/Despiked_LVDT_Data.csv"
...
STRESS_SMOOTH="$PDDIR/1_cleanup/Smoothed_Pressure_Data.csv"
LVDT_SMOOTH="$PDDIR/1_cleanup/Smoothed_LVDT_Data.csv"
...
SPLIT_DATA_PATH="$PDDIR/2_split_data"
PADSEC=21600
...
python $SRC/primary/analyze_cleaned_lvdt.py -i $SPLIT_DATA_PATH -o "$PDDIR/3_lvdt_melt_corrected" -d $PADSEC

echo "!!!!!!!!!! TIME SERIES PROCESSING COMPLETE !!!!!!!!!!"

# Cavity pick postprocessing
echo "~~~~~~~~~~ PROCESSING CAVITY GEOMETRIES ~~~~~~~~~~"
python $SRC/preprocessing/process_cavity_picks.py -i '..' -o "$PDDIR/1_cleanup"
python $SRC/primary/analyze_cavity_geometry.py -i "$PDDIR/1_cleanup" -o "$RDIR"
