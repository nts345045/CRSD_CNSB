...
if [ ! -d $PDDIR ]; then
    echo "$PDDIR does not exist, making directory"
    mkdir -p $PDDIR
fi
...
STRESS_PUTC_FILE="$PDDIR/1_PhysicalUnitsData__UTC_Timing.csv"
...
LVDT_UTC_FILE="$PDDIR/1_Stitched_LVDT_Data__UTC_Timing.csv"
...
STRESS_RESAMPLED="$PDDIR/2_Resampled_Pressure_Data.csv"
LVDT_RESAMPLED="$PDDIR/2_Resampled_LVDT_Data.csv"
...
STRESS_DESPIKED="$PDDIR/3_Despiked_Pressure_Data.csv"
LVDT_DESPIKED="$PDDIR/3_Despiked_LVDT_Data.csv"
...
STRESS_SMOOTH="$PDDIR/4_Smoothed_Pressure_Data.csv"
LVDT_SMOOTH="$PDDIR/4_Smoothed_LVDT_Data.csv"
...
SPLIT_DATA_PATH="$PDDIR/5_split_data"
PADSEC=21600
...
# python $SRC/primary/analyze_cleaned_lvdt.py -i $SPLIT_DATA_PATH -o "$PDDIR/6_lvdt_melt_corrected" -d $PADSEC
...
# python $SRC/primary/analyze_cavity_geometry.py -i $PDDIR -o "$PDDIR/cavities"
