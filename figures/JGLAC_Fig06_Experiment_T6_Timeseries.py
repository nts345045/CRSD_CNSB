...
# Map Experimental Data Files
T06_NT = os.path.join(DDIR,'5_split_data','EX_T06-Pressure.csv')
T06_LV = os.path.join(DDIR,'6_lvdt_melt_corrected','EX_T06-LVDT-reduced.csv')
T06_CM = os.path.join(DDIR,'cavities','experiment_T06_cavity_metrics.csv')
T06_CP = os.path.join(DDIR,'cavities','Postprocessed_Contact_Geometries.csv')
# Map output directory
...
df_CM = pd.read_csv(T06_CM,parse_dates=True,index_col=[0])
df_CP = pd.read_csv(T06_CP,parse_dates=True,index_col=[0])

df_NT.index = pd.to_datetime(df_NT.Epoch_UTC,unit='s')
df_Z.index = pd.to_datetime(df_Z.Epoch_UTC, unit='s')
df_CM.index += pd.Timedelta(5,unit='hour')
df_CP.index += pd.Timedelta(5,unit='hour')
...
if issave:
	OFILE = os.path.join(ODIR,'JGLAC_Fig06_Experiment_T06_Timeseries_%ddpi.%s'%(DPI,FMT.lower()))
