...
def main(args):
    ...
    T24_DAT = os.path.join(args.input_path,'EX_T24_cavity_metrics.csv')
    T06_DAT = os.path.join(args.input_path,'EX_T06_cavity_metrics.csv')
    ...
    axs[0].set_xlabel('Elapsed Time from Start of Exp. T24 [hr]')
    ...
    if not args.render_only:
        if args.dpi == 'figure':
            ...
            try:
                ...
                dpi = 'figure'
        if dpi == 'figure':
            savename = os.path.join(args.output_path, f'JGLAC_Fig08_fdpi.{args.format}')
        else:
            savename = os.path.join(args.output_path, f'JGLAC_Fig08_{dpi}dpi.{args.format}')
