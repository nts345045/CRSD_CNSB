...
if __name__ == '__main__':
	...
	parser.add_argument(
		...
		action='store',
		dest='fname',
		default='../results/figures/JGLAC_Fig01c',
		help='path and name to save the rendered figure to, minus extension (use -x for extension). Defaults to "../results/figures/JGLAC_Fig01c"',
		type=str
	)
	...
	parser.add_argument(
		...
		default='png',
		choices=['png','pdf','svg'],
		help='the figure output extension (e.g., *.png, *.pdf, *.svg) callable by :meth:`~matplotlib.pyplot.savefig`. Defaults to "png"',
		type=str
	)
	...
	if args.dpi == 'figure':
		...
		try:
			...
			dpi = 'figure'
	if dpi == 'figure':
		savename = f'{args.fname}_fdpi.{args.extension}'
	else:
		savename = f'{args.fname}_{dpi:.2f}dpi.{args.extension}'

	if not os.path.exists(os.path.split(savename)[0]):
		os.makedirs(os.path.split(savename)[0])
	plt.savefig(savename, dpi=dpi, format=args.extension)
